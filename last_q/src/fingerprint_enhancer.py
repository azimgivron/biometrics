...
class FingerprintImageEnhancer(object):
    ...
    def ridge_segment(self, img):
        ...
        return (self._normim, self._mask)

    def ridge_orient(self):
        ...
        denom = (
            np.sqrt(np.power(Gxy, 2) + np.power((Gxx - Gyy), 2)) + np.finfo(float).eps
        )
    ...
    def ridge_freq(self):
        ...
        for r in range(0, rows - self.ridge_freq_blksze, self.ridge_freq_blksze):
            for c in range(0, cols - self.ridge_freq_blksze, self.ridge_freq_blksze):
                blkim = self._normim[r : r + self.ridge_freq_blksze][
                    :, c : c + self.ridge_freq_blksze
                ]
                blkor = self._orientim[r : r + self.ridge_freq_blksze][
                    :, c : c + self.ridge_freq_blksze
                ]

                freq[r : r + self.ridge_freq_blksze][
                    :, c : c + self.ridge_freq_blksze
    ...
    def __frequest(self, blkim, blkor):
        ...
        if cols_maxind < 2:
            ...
            if (
                waveLength >= self.min_wave_length
                and waveLength <= self.max_wave_length
            ):
    ...
    def ridge_filter(self):
        ...
        for o in range(0, angleRange):
            ...
            rot_filt = scipy.ndimage.rotate(
                reffilter, -(o * self.angleInc + 90), reshape=False
            )
        ...
        return self._binim, gabor_filter
    ...
    def enhance(self, img, resize=False):
        ...
        self.ridge_segment(img)  # normalise the image and find a ROI
        self.ridge_orient()  # compute orientation image
        self.ridge_freq()  # compute major frequency of ridges
        self.ridge_filter()  # filter the image using oriented gabor filter
