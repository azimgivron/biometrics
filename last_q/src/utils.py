...
def show(*images, enlarge_small_images=True, max_per_row=-1, font_size=0):
    ...
    def convert_for_display(img):
        if img.dtype != np.uint8:
            ...
            img = np.clip(offset + mult * (img.astype(float)) / d, 0, 255).astype(
                np.uint8
            )
        return img
        ...
        if type(img) == str:
            ...
            if enlarge_small_images:
                ...
                if h < REF_SCALE or w < REF_SCALE:
                    ...
                    img = cv.resize(
                        img, (w * scale, h * scale), interpolation=cv.INTER_NEAREST
                    )
            data = "data:image/png;base64," + base64.b64encode(
                cv.imencode(".png", img)[1]
            ).decode("utf8")
        return data, title
    ...
    for r in rows:
        ...
        html_content += "".join(
            ["<table><tr>"]
            + [
                f"<td style='text-align:center;{font}'>{html.escape(t)}</td>"
                for _, t in l
            ]
            + ["</tr><tr>"]
...
def draw_orientations(
    fingerprint, orientations, strengths, mask, scale=3, step=8, border=0
):
    ...
    s = np.round(-np.sin(orientations) * strengths * d * sd).astype(
        int
    )  # minus for the direction of the y axis
    thickness = 1 + scale // 5
...
def draw_minutiae(
    fingerprint, minutiae, termination_color=(255, 0, 0), bifurcation_color=(0, 0, 255)
):
    ...
    return res  # cv.resize(res, (res.shape[1]*2, res.shape[0]*2), interpolation = cv.INTER_NEAREST)
...
def angle_mean(a, b):
    return math.atan2(
        (math.sin(a) + math.sin(b)) / 2, ((math.cos(a) + math.cos(b)) / 2)
    )


# Utility functions for MCC
def draw_minutiae_and_cylinder(
    fingerprint, origin_cell_coords, minutiae, values, i, show_cylinder=True
):
    ...
    if show_cylinder:
        for v, (cx, cy) in zip(
            values[i], _compute_actual_cylinder_coordinates(*minutiae[i])
        ):
            cv.circle(
                res,
...
def draw_match_pairs(
    f1, m1, v1, f2, m2, v2, cells_coords, pairs, i, show_cylinders=True
):
    ...
    res[:h1, :w1] = draw_minutiae_and_cylinder(
        f1, cells_coords, m1, v1, p1[i], show_cylinders
    )
    res[:h2, w1 : w1 + w2] = draw_minutiae_and_cylinder(
        f2, cells_coords, m2, v2, p2[i], show_cylinders
    )
