...
def canny(im, sigma, vert, horz):
    ...
    h = np.concatenate([im[:, 1:cols], np.zeros([rows, 1])], axis=1) - np.concatenate(
        [np.zeros([rows, 1]), im[:, 0 : cols - 1]], axis=1
    )

    v = np.concatenate([im[1:rows, :], np.zeros([1, cols])], axis=0) - np.concatenate(
        [np.zeros([1, cols]), im[0 : rows - 1, :]], axis=0
    )
...
def nonmaxsup(in_img, orient, radius):
    ...
    col, row = np.meshgrid(
        np.arange(iradius, cols - iradius), np.arange(iradius, rows - iradius)
    )
